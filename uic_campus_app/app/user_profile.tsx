...
const TAB_LABELS=['Feed', 'Repost/Comments', 'Media'];
...
export default function UserProfile() {
  const userInfoAnim = useRef(new Animated.Value(0)).current; // Animation for UserInfo
  ...
  const handleScroll = (event: any) => {
    ...
    const userInfoHeight = 180; // Based on your UserInfo component (padding + content)
    
    if (offsetY > lastOffsetY + 10 && !hidden) {
      // Scroll down: hide UserInfo only
      Animated.timing(userInfoAnim, { 
        toValue: -userInfoHeight, 
        duration: 250, 
        useNativeDriver: true 
      }).start();
      setHidden(true);
    } else if (offsetY < lastOffsetY - 10 && hidden) {
      // Scroll up: show UserInfo
      Animated.timing(userInfoAnim, { 
        toValue: 0, 
        duration: 250, 
        useNativeDriver: true 
    ...
    <>
      <View style={{ flex: 1 }}>
        ...
        <Animated.View style={{ 
          ...
          left: 0, 
          right: 0, 
          transform: [{ translateY: userInfoAnim }], 
          zIndex: 12,
          backgroundColor: '#fff'
        }}>
          <UserInfo/>
            ...
            {TAB_LABELS.map((label, idx) => (
              <TouchableOpacity
                ...
                style={{ 
                  flex: 1, 
                  padding: 9, 
                  borderBottomWidth: tabIndex === idx ? 2 : 0, 
              ...
              >
                <Animated.Text style={{ 
                  textAlign: 'center', 
                  color: tabIndex === idx ? 'dodgerblue' : '#888', 
        ...
        {/* <Animated.View style={{ 
          position: "absolute", 
          top: 180, // Height of UserInfo component
          left: 0, 
          right: 0, 
          zIndex: 11,
          backgroundColor: '#fff',
            ...
            <View key={tab} style={{ flex: 1 }}>
              <ScrollView
                ...
                contentContainerStyle={{ 
                  paddingTop: hidden ? 40 : 190, // Dynamic padding: 40 (TabBar only) or 220 (UserInfo + TabBar)
                  paddingBottom: 60, 
                  minHeight: '100%' 
                }}
